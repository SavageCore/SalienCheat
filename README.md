...
0. Add tokens to `token.txt` with one token per line (making sure to use LF line endings)
...
`cheat.sh` will check for updates and any new or not running tokens in `token.txt` each run.
